export default {
  rules: {
    phone: [
      ...
      v=> /^\+?([0-9]{2})\)?[-. ]?([0-9]{4})[-. ]?([0-9]{4})$/.test(v) || "شماره تماس درست را واردنماید."
    ],
    required_phone: [
      v => !!v || ".وارد کردن شماره تماس الزامی می باشد ",
      //v => /^[0-9]/.test(v) || ".شماره تماس وارد شده باید اعداد باشد",
      v => (v || " ").length == 14 || ".شماره تماس بایدعدد ده رقمی باشد",
      //v=> /^\+?([0-9]{2})\)?[-. ]?([0-9]{4})[-. ]?([0-9]{4})$/.test(v) || "شماره تماس درست را واردنماید."
    ...
    tazkira_id: [
      ...
      v => /^[-]*$/.test(v) || "فورمت واردکردن شماره تذکره درست نمی باشد. باشد توسط دش(-) از هم جدا شود.",
      v => (v || " ").length ==15 || "شماره تذکره بابد 15 رقم باشد."
    ],
    required_number: [
      v => !!v || ".وارد کردن این فیلد الزامی می باشد",
